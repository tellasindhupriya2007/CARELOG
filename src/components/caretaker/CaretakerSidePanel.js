...
import { Clock, ShieldCheck, Activity, Bell, AlertTriangle, CheckCircle2 } from 'lucide-react';

export default function CaretakerSidePanel() {
    const navigate = useNavigate();
    const { patientId } = useAuthContext();
    const [patientInfo, setPatientInfo] = useState({ name: 'Loading...', status: 'Stable', lastUpdated: 'Just now' });
            ...
            try {
                ...
                if (pDoc.exists()) {
                    ...
                    setPatientInfo({
                        ...
                        status: data.healthDetails?.status || 'Stable',
                        lastUpdated: 'Today'
                    });
                }
            } catch (e) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100%' }}>
            ...
            {/* C. Patient Info */}
            <Card style={{ padding: '24px', backgroundColor: colors.white }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                    <div style={{ width: '56px', height: '56px', borderRadius: '16px', backgroundColor: colors.lightBlue, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <ShieldCheck size={28} color={colors.primaryBlue} />
                    </div>
                    <div>
                        <h3 style={{ fontSize: '18px', fontWeight: '900', color: colors.textPrimary, marginBottom: '4px' }}>{patientInfo.name}</h3>
                        <span style={{ fontSize: '13px', fontWeight: '800', color: patientInfo.status === 'Critical' ? colors.alertRed : colors.primaryGreen, backgroundColor: patientInfo.status === 'Critical' ? '#FEE2E2' : '#DCFCE7', padding: '4px 10px', borderRadius: '8px', textTransform: 'uppercase' }}>
                            {patientInfo.status}
                        </span>
                    </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: colors.textSecondary, fontSize: '12px', fontWeight: '600' }}>
                    <Clock size={14} />
                    Last updated: {patientInfo.lastUpdated}
                </div>
            </Card>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    <div style={{ position: 'relative', width: '64px', height: '64px', flexShrink: 0 }}>
                        ...
                        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                            <span style={{ fontSize: '15px', fontWeight: '900', color: colors.primaryBlue }}>{Math.round(progress.percent)}%</span>
                        </div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <span style={{ fontSize: '24px', fontWeight: '900', color: colors.textPrimary, lineHeight: 1 }}>{progress.completed}/{progress.total}</span>
                        <span style={{ fontSize: '13px', color: colors.textSecondary, fontWeight: '600' }}>Tasks Completed</span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ backgroundColor: colors.white, padding: '24px', borderRadius: '16px', border: `1px solid ${colors.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    {alerts.length > 0 && (
                        <span style={{ fontSize: '12px', fontWeight: '800', color: colors.alertRed, backgroundColor: '#FEF2F2', padding: '4px 8px', borderRadius: '6px' }}>
                            {alerts.length} NEW
                        </span>
                    )}
                </div>
                        ...
                        alerts.map(a => (
                            <div key={a.id} style={{ display: 'flex', gap: '12px', padding: '12px', backgroundColor: a.type === 'critical' ? '#FEF2F2' : '#FFFBEB', borderRadius: '12px', border: `1px solid ${a.type === 'critical' ? '#FEE2E2' : '#FEF3C7'}` }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    ...
                                    <span style={{ fontSize: '11px', fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase' }}>{a.type}</span>
                                </div>
                            </div>
                        ))
                    ) : (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '16px', backgroundColor: '#ECFDF5', borderRadius: '12px', border: '1px solid #D1FAE5' }}>
                            <CheckCircle2 size={18} color="#10B981" />
                            <span style={{ fontSize: '13px', fontWeight: '700', color: '#065F46' }}>No active alerts. Patient stable.</span>
                        </div>
                    )}
                </div>
            </div>
