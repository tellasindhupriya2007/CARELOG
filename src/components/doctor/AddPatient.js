...
export default function AddPatient() {
    ...
    const [form, setForm] = useState({
        name: '', age: '', gender: '', dob: '', bloodGroup: '', address: '',
    ...
    const handleLink = async () => {
        if (!linkId.trim()) { setError('Please enter a Patient ID.'); return; }
        setSaving(true);
        setError(null);
        setLoadingMsg('Linking profile...');
        try {
            const { linkById: lb } = await import('../../services/patientService');
            const firestoreId = await lb(linkId, user.uid, 'doctor');
            setSuccess(firestoreId);
        } catch (e) {
            setError(e.message || 'ID not found. Please verify with the family.');
        } finally {
            setSaving(false);
            setLoadingMsg('');
        }
            ...
            <DoctorShell alertCount={alertCount}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: DS.surface }}>
                    <div style={{ textAlign: 'center', padding: '48px', maxWidth: '480px' }}>
                        ...
                        <h2 style={{ fontSize: '26px', fontWeight: '900', color: DS.textPrimary, margin: '0 0 8px', letterSpacing: '-0.5px' }}>{mode === 'link' ? 'Profile Linked!' : 'Patient Created!'}</h2>
                        <p style={{ fontSize: '14px', color: DS.textMuted, margin: '0 0 8px', fontWeight: '500', lineHeight: 1.6 }}>
                            {mode === 'link' ? 'Successfully connected to existing clinical records.' : (
                        ...
                        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                            ...
                            <button
                                onClick={() => { setSuccess(null); setMode('create'); setLinkId(''); setForm({ name: '', age: '', gender: '', dob: '', bloodGroup: '', address: '', conditions: '', allergies: '', medications: '', notes: '', emergencyContact: '', emergencyPhone: '' }); setActiveSection('identity'); }}
                            ...
                            >
                                {mode === 'link' ? 'Link Another' : 'Add Another'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', backgroundColor: DS.surface, padding: '32px' }}>
                <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: '26px', fontWeight: '900', color: DS.textPrimary, margin: 0, letterSpacing: '-0.5px' }}>
                                {mode === 'create' ? 'Add New Patient' : 'Link Existing Patient'}
                            </h1>
                            <p style={{ fontSize: '13px', color: DS.textMuted, fontWeight: '500', margin: '4px 0 0' }}>
                                {mode === 'create' ? 'Create a fresh profile with new Patient ID' : 'Link to a profile already created by a family member'}
                            </p>
                        </div>
                    </div>

                    <div style={{ display: 'flex', backgroundColor: DS.surfaceLowest, padding: '4px', borderRadius: '14px', marginBottom: '24px', width: 'fit-content', border: `1px solid ${DS.outlineVariant}` }}>
                        <button onClick={() => setMode('create')} style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', backgroundColor: mode === 'create' ? DS.primaryContainer : 'transparent', color: mode === 'create' ? 'white' : DS.textSecondary, fontSize: '13px', fontWeight: '800', cursor: 'pointer', transition: 'all 0.2s' }}>
                            Create Profile
                        </button>
                        <button onClick={() => setMode('link')} style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', backgroundColor: mode === 'link' ? DS.primaryContainer : 'transparent', color: mode === 'link' ? 'white' : DS.textSecondary, fontSize: '13px', fontWeight: '800', cursor: 'pointer', transition: 'all 0.2s' }}>
                            Link via ID
                        </button>
                    </div>

                    {error && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#FEF2F2', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '14px', padding: '14px 18px', marginBottom: '24px' }}>
                    ...
                    {mode === 'link' ? (
                        <div style={{ backgroundColor: DS.surfaceLowest, borderRadius: '24px', padding: '40px', boxShadow: '0 4px 16px rgba(25,28,30,0.05)', textAlign: 'center' }}>
                            <div style={{ width: '64px', height: '64px', borderRadius: '20px', backgroundColor: '#EEF2FF', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                                <Save size={28} color={DS.primaryContainer} />
                            </div>
                            <h3 style={{ fontSize: '20px', fontWeight: '900', color: DS.textPrimary, marginBottom: '8px' }}>Assign Patient ID</h3>
                            <p style={{ fontSize: '14px', color: DS.textMuted, marginBottom: '32px', maxWidth: '320px', margin: '0 auto 32px' }}>
                                Enter the unique Patient ID (e.g., CL-2026-1234) provided by the patient's family to link their charts to your dashboard.
                            </p>
                            
                            <div style={{ maxWidth: '400px', margin: '0 auto' }}>
                                <input 
                                    type="text"
                                    placeholder="CL-YYYY-XXXX"
                                    value={linkId}
                                    onChange={e => setLinkId(e.target.value.toUpperCase())}
                                    style={{ ...inputStyle, textAlign: 'center', fontSize: '24px', fontWeight: '900', letterSpacing: '2px', padding: '16px', marginBottom: '20px', border: `2px solid ${DS.primaryContainer}30` }}
                                />
                                <button onClick={handleSubmit} disabled={saving} style={{ width: '100%', padding: '16px', borderRadius: '14px', border: 'none', background: `linear-gradient(135deg, ${DS.primary}, ${DS.primaryContainer})`, color: 'white', fontSize: '15px', fontWeight: '800', cursor: 'pointer', boxShadow: `0 8px 24px ${DS.primaryContainer}40` }}>
                                    {saving ? 'Linking Profile...' : 'Link to Dashboard'}
                                </button>
                            </div>
                        </div>
                    ) : (
                        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px', alignItems: 'start' }}>
                            ...
                            <div style={{ backgroundColor: DS.surfaceLowest, borderRadius: '24px', padding: '32px', boxShadow: '0 4px 16px rgba(25,28,30,0.05)' }}>
                                {activeSection === 'identity' && (
                                    <>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
                                            ...
                                            <h2 style={{ fontSize: '18px', fontWeight: '900', color: DS.textPrimary, margin: 0 }}>Basic Identity</h2>
                                        </div>
